...
# Script name
#
SCRIPT = "bin-packer-stops.py"
SCRIPT_URL = "https://gist.githubusercontent.com/matteo-s/82ba623b72c8c1567e8c76af447a72ca/raw/bin_packer-stops.py"
...
# Pool settings
#
POOL_ID = 'BinPackStops'
DEDICATED_POOL_NODE_COUNT = 0
LOW_PRIORITY_POOL_NODE_COUNT = 15
...
JOB_ID = "{}_v8_{}-{}-{}".format(POOL_ID, COUNTRY,
...
if __name__ == '__main__':
    ...
    path_in = f"stoplocation-v8_prefix_r70-s5-a70-h6/{COUNTRY}/"
    path_out = f"stoplocation-v8_r70-s5-a70-h6/country={COUNTRY}/"
    ...
    for cur_date in dates:
        ...
        for state in US_STATES:
            ...
            packs.append({
                ...
                'day': cur_date,
                'state': state,
                'out': f"{path_out}{cur_partition}state={state}/",
                'key': f"{cur_partition}state={state}/00000",
                'part': 0,
                'dedup': DEDUP
            })
        ...
        try:
            ...
            for pack in packs:
                tid = "task_{}_{}".format(
                    pack['day'].strftime("%Y%m%d"), pack['state'])

                commands = "/bin/bash -c \"python3 {} --storage '{}' --sas '{}' --container-in '{}' --container-out '{}'  --path-out '{}' --basekey '{}' --part-num {} --dedup-columns '{}' --state '{}' --path-in '{}' \"".format(
                    script_file,
                    STORAGE_ACCOUNT_NAME, SAS_TOKEN, INPUT_CONTAINER_NAME, OUTPUT_CONTAINER_NAME,
                    pack['out'], pack['key'], pack['part'], pack['dedup'],
                    pack['state'],  pack['path'])
